    ...
    <el-form :model="jobTransfer" :rules="rules" ref="jobTransfer" label-width="100px" class="demo-ruleForm">
      ...
      <el-form-item label="调动岗位" prop="afterJob">
        <el-select v-model="jobTransfer.afterJob" placeholder="请选择活动区域">
          <el-option v-for="item in job" :key="item.jId" :label="item.jobName" :value="item.jId"></el-option>
        </el-select>
      </el-form-item>
  ...
  created() {
    if (this.eId) {
      ...
      this.get("emp/getOneTransfer",data => {
          this.ruleForm = data;
          this.jobTransfer.empId=this.ruleForm.eId;
          this.jobTransfer.beforeJob=this.ruleForm.jobId;
          this.getTime();
