    ...
    <el-form :model="deptTransfer" :rules="rules" ref="deptTransfer" label-width="100px" class="demo-ruleForm">
      ...
      <el-form-item label="调动部门" prop="afterDept">
        <el-select v-model="deptTransfer.afterDept" placeholder="请选择活动区域">
          <el-option v-for="item in dept" :key="item.id" :label="item.deptName" :value="item.id"></el-option>
        </el-select>
      </el-form-item>
  ...
  created() {
    if (this.id) {
      ...
      this.get("emp/getOneTransfer",data => {
          this.ruleForm = data;
          this.deptTransfer.empId=this.ruleForm.id;
          this.deptTransfer.beforeDept=this.ruleForm.dept;
          this.getTime();
