<template>
  <div>
    ...
    <el-table :data="tableData.list" border style="width: 100%" align="center">
      ...
      <el-table-column prop="job.jobName" label="原岗位名称" width="120"></el-table-column>
      <el-table-column prop="afterJob" label="新岗位名称" width="120" :formatter="nameformat"></el-table-column>
...
export default {
  ...
  methods: {
    ...
    nameformat(row, column, cellValue, index) {
      if (cellValue == 1) return "Java开发工程师";
      if (cellValue == 2) return "资深保洁员";
      if (cellValue == 3) return "保安";
      if (cellValue == 4) return "保洁";
      if (cellValue == 5) return "咸鱼";
      else return "其他岗位";
    },
    changePageNo(i) {
    ...
    export2Excel() {
      require.ensure([], () => {
        ...
        export_json_to_excel(tHeader, filterVal, "岗位调动员工excel"); //对应下载文件的名字
      });
    },
